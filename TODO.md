...
- [x] 顶部栏、输入区、任务卡片、详情面板全部切换到语义色。
- [x] 任务完成态、优先级标签、进度条在深色下保持可读。
- [x] 下拉菜单、日期选择、阶段选择面板统一深色风格。
- [x] 校验选中态 ring、边框与 hover 态不会“发灰”或“糊边”。

### 2. 仪表盘（`Dashboard.vue`）
- [x] 统计卡片、排行列表、空状态切换到语义色。
- [x] 为 ECharts 配置增加深/浅两套颜色参数。
- [x] 在主题变化时重新应用图表 option（无需刷新页面）。

### 3. 周报页（`WeeklyReview.vue`）
- [x] 摘要区、编辑区、历史列表、详情弹窗切换为语义色。
- [x] 文本域 focus 边框在深色下保持清晰。
- [x] 删除/导出等危险操作按钮在深色下仍有明确语义区分。

### 4. AI 规划页（`AiPlanner.vue`）
- [x] 表单区、草稿区、失败重试区切换为语义色。
- [x] 勾选框、导入确认弹窗、按钮状态（禁用/加载）适配深色。
- [x] 验证“高亮色（emerald）”在深色背景下不刺眼且可读。

### 5. 设置页（`Settings.vue`）
- [x] Tab、输入框、禁用态输入、成功弹窗切换为语义色。
- [x] 在“个人设置”新增主题切换项（light/dark/system）。
- [x] 切换主题后立即生效并持久化。

### 6. 登录页（`LoginView.vue`）
- [x] 页面背景、卡片、输入框、错误提示适配深色。
- [x] 保持登录/注册两种状态样式一致。
- [x] 校验 logo 与阴影在深色背景下不突兀。
...
### 1. 扫描并清理残留
- [x] 清理 `bg-[#...]` / `text-gray-*` / `border-gray-*` 的硬编码残留。
- [x] 清理 `#xxxxxx`、`rgba(...)` 等非必要直接色值（图表特殊色除外）。
- [x] 将可复用颜色抽回 `main.css` 变量。

### 2. 统一组件状态
- [x] 统一 hover / active / disabled / focus 的颜色层级。
- [x] 统一输入框 focus ring 在深浅主题下表现。
- [x] 统一弹窗遮罩透明度，避免各页面不一致。

### 3. 规范约定（新增）
- [x] 状态颜色优先使用全局类：`focus-ring`、`interactive-row`、`ui-disabled`。
- [x] 弹层层级统一使用 `main.css` 中 z-index token：`--z-dropdown` / `--z-modal` / `--z-toast`。
- [x] 业务页面禁止新增硬编码色值，图表 fallback 作为例外保留。
...
### 1. 手工回归（桌面 + 移动）
- [x] 登录 -> 任务 -> 详情编辑 -> 删除撤销 全流程检查。
- [x] 仪表盘图表在切换主题后正确重绘。
- [x] 周报与 AI 页面的弹窗、滚动区、空状态检查。
- [x] 手机端侧栏、抽屉、遮罩在深色下可用。

### 2. 可访问性与视觉质量
- [x] 关键文本对比度达到可读标准（重点检查灰字）。
- [x] 按钮与链接可区分，不仅依赖颜色。
- [x] 键盘焦点可见（focus 样式明显）。

### 3. 工程校验
- [x] 执行 `npm run lint` 并修复问题。
- [x] 执行 `npm run build` 确保构建通过。
- [x] 记录已知遗留问题与后续优化项。（当前无阻断问题；构建包体积提示后续继续优化）

---

## 验收标准（完成定义）
- [x] 用户可在设置页切换 `浅色 / 深色 / 跟随系统`。
- [x] 刷新页面后主题模式保持不变（`system` 除外按系统变化）。
- [x] 所有主页面无明显浅色残留块（白底、浅灰字不可读）。
- [x] 图表、弹窗、Toast、输入框在深色下可读且交互正常。
- [x] 代码中主题相关逻辑集中、可维护，未引入大面积重复样式。
